...
-- ROWS COUNT FOR TABLE DIRECTOR_MAPPING 
SELECT 
...
-- ROWS COUNT FOR TABLE GENRE 
SELECT 
	COUNT(*) AS total_records 
FROM 
	genre; 
-- rows count are 14662

-- ROWS COUNT FOR TABLE MOVIE 
SELECT 
	COUNT(*) AS total_records 
FROM 
	movie; 
-- rows counts are 7997
-- ROWS COUNT FOR TABLE NAMES
SELECT 
	COUNT(*) AS total_records 
FROM 
	names; 
-- rows count are 25735
...
SELECT 
    ...
    END)  AS null_counts
FROM movie;
-- worldwide column has 3724 null values 
...
-- The highest movies were produced in the month of march.
...
-- Total 1059 movies were produced in the year of 2019
...
-- The highest number of movies were produced by Drama genre.
...
-- there are total  3289 movies belongs to only one genre 
...
SUM(total_votes) AS vote_counts,
